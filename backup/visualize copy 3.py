...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  .price-range-row.last b { color:#bfdbfe; }
  .release-low-badge {
    display:inline-flex;
    align-items:center;
    margin-top:10px;
...
function priceRangeHtml(r, points=[]) {
  const lastToken = num(r.hist_last) !== null
    ? r.hist_last
    : ((points || []).length ? points[points.length - 1].price : null);
  const last = tokenUsdPair(lastToken, r);
  const low = tokenUsdPair(r.hist_min, r);
  const high = tokenUsdPair(r.hist_max, r);
  return `<div class="price-range">
    <div class="price-range-row last"><span>Last</span><div><b>${esc(last.usd)}</b><small>${esc(last.tokens)} tokens</small></div></div>
