...
<div class="app">
  <header class="topbar">
    <div>
      <h1>CS2 Sticker Decision Dashboard</h1>
      <p class="sub">Analyzer output with Paper, Foil, Holo and Gold coverage. Use the filters to isolate a variant or sticker type, then sort by price, expected return, confidence, flood risk or manual score.</p>
    </div>
    <div class="stats">
      <div class="stat"><span>Visible</span><b id="visibleCount">0</b></div>
      <div class="stat"><span>Total</span><b id="totalCount">0</b></div>
  ...
  <main class="content">
    <section class="panel">
      ...
      <div class="footer-note">Generated files are written under <code>visualized/</code>. Trend lines use real collected history when available; dashed lines mark snapshot/range fallback data.</div>
    </section>
  </main>
...
function rowHtml(r) {
  ...
  return `<tr class="${atReleaseLow ? 'release-low-row' : ''}">
    ...
    <td>
      ...
      ${atReleaseLow ? '<div class="release-low-badge">Current low</div>' : ''}
      ${priceRangeHtml(r)}
